...
unsafe extern "C" fn wrap(argc: c_int, stack: ffi::py_StackRef) -> bool {
    let Some((text, width)) = text_and_optional_width(argc, stack) else {
        return false;
    };
...
unsafe extern "C" fn fill(argc: c_int, stack: ffi::py_StackRef) -> bool {
    let Some((text, width)) = text_and_optional_width(argc, stack) else {
        return false;
    };
...
unsafe extern "C" fn dedent(argc: c_int, stack: ffi::py_StackRef) -> bool {
...
unsafe extern "C" fn indent(argc: c_int, stack: ffi::py_StackRef) -> bool {
    // SAFETY: PocketPy supplies an active callback stack containing `argc` values.
    let arguments = unsafe { Arguments::from_raw(argc, stack) };
...
unsafe extern "C" fn shorten(argc: c_int, stack: ffi::py_StackRef) -> bool {
    // SAFETY: PocketPy supplies an active callback stack containing `argc` values.
    let arguments = unsafe { Arguments::from_raw(argc, stack) };
    if !arguments.require_arity(2, 2) {
        return false;
    ...
    let Some(width) = arguments
        .get(1)
        .and_then(super::super::native::Value::integer)
    else {
...
fn text_and_optional_width(argc: c_int, stack: ffi::py_StackRef) -> Option<(String, i64)> {
    // SAFETY: PocketPy supplies an active callback stack containing `argc` values.
    let arguments = unsafe { Arguments::from_raw(argc, stack) };
