...
unsafe extern "C" fn compress(argc: c_int, stack: ffi::py_StackRef) -> bool {
    let Some(input) = bytes_argument(argc, stack) else {
        return false;
    };
    ...
    encoder.finish().map_or_else(
        |_| runtime_error(c"gzip compression failed"),
        |output| return_bytes(&output),
    )
}

unsafe extern "C" fn decompress(argc: c_int, stack: ffi::py_StackRef) -> bool {
    let Some(input) = bytes_argument(argc, stack) else {
        return false;
    };
...
unsafe extern "C" fn inflate_raw(argc: c_int, stack: ffi::py_StackRef) -> bool {
    let Some(input) = bytes_argument(argc, stack) else {
        return false;
    };
...
fn decode_limited(mut decoder: impl Read) -> bool {
    ...
    match decoder.by_ref().take(67_108_865).read_to_end(&mut output) {
...
fn bytes_argument(argc: c_int, stack: ffi::py_StackRef) -> Option<Vec<u8>> {
