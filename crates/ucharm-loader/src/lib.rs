...
use md5::{Digest, Md5};
use ucharm_format::{DecodeError, LayoutError, TRAILER_SIZE, Trailer};
...
pub fn prepare_path(bundle_path: &Path, cache_root: &Path) -> Result<PreparedBundle, LoaderError> {
    ...
    let cache_dir = cache_root.join(format!("ucharm-{}", metadata.cache_key()));
...
mod tests {
    ...
    fn rejects_a_symlinked_cache_directory() {
        ...
        symlink(
            ...
            temporary
                .path
                .join(format!("ucharm-{}", metadata.cache_key())),
        )
    ...
    impl TestDirectory {
        fn new(name: &str) -> Self {
            ...
            let path = std::env::temp_dir().join(format!(
                "ucharm-loader-test-{}-{counter}-{name}",
                std::process::id()
            ));
