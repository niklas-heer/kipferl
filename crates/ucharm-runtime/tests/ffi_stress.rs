...
fn keeps_callback_values_rooted_across_allocation_and_exception_stress() {
    let output = run(
        concat!(
            "import ansi, args, base64, binascii, charm, collections, copy, csv, dataclasses, datetime, errno, fnmatch, functools, heapq, input, io, itertools, json, operator, random, statistics, term, textwrap, typing, uuid\n",
            "spec = {\n",
            "    '--name': str,\n",
            "    '--count': (int, 0),\n",
            "    '--verbose': bool,\n",
