...
[[ -x "$demo_cli" ]] || { echo "Set KIPFERL_DEMO_CLI to a published v0.7.0 executable for this host." >&2; exit 1; }
case "$demo_cli" in /*) ;; *) demo_cli="$repo_root/$demo_cli" ;; esac
version=$(NO_COLOR=1 "$demo_cli" --version | sed $'s/\033\\[[0-9;]*m//g')
[[ "$version" == 'Kipferl v0.7.0' ]] || { echo "Expected published Kipferl v0.7.0, got: $version" >&2; exit 1; }
...
sed "s|website/public/demos/kipferl-0.7|$DEMO_ROOT/kipferl-0.7|g" demo.tape > "$DEMO_ROOT/demo.tape"
...
ffmpeg -hide_banner -loglevel error -y -ss "${KIPFERL_DEMO_POSTER_SECOND:-27}" \
    -i "$DEMO_ROOT/kipferl-0.7.mp4" -frames:v 1 "$DEMO_ROOT/kipferl-0.7.png"
cwebp -quiet -lossless "$DEMO_ROOT/kipferl-0.7.png" -o "$DEMO_ROOT/kipferl-0.7.webp"
ffmpeg -hide_banner -loglevel error -y -i "$DEMO_ROOT/kipferl-0.7.mp4" \
    -c copy -movflags +faststart "$DEMO_ROOT/kipferl-0.7-stream.mp4"
mv "$DEMO_ROOT/kipferl-0.7-stream.mp4" website/public/demos/kipferl-0.7.mp4
mv "$DEMO_ROOT/kipferl-0.7.webp" website/public/demos/kipferl-0.7.webp
ffprobe -v error -show_entries format=duration,size -show_entries stream=width,height \
    -of json website/public/demos/kipferl-0.7.mp4
