...
system Python installation in the published **v0.7.0-rc.1 release candidate**.
[Install the candidate](/docs/getting-started/installation#release-candidate)
to use these commands; stable v0.6.0 predates the package manager.
...
Each v0.7.0-rc.1 release CLI includes fresh `tzdata==2025.2` evidence for its
exact runtime on macOS Apple Silicon, macOS Intel, Linux x86_64, or Linux ARM64.
Its checks cover version constants and the `TZif` headers of
four timezone data files. This is evidence for loading package resources;
it does not establish timezone conversion support or provide `zoneinfo`.

With the release-candidate CLI and a project, install the exact version:
...
## Upgrade to 0.7.0-rc.1

An existing `kipferl.lock` identifies the exact runtime binary and platform.
Switching from a development build or another target can invalidate it even
when the visible version matches. Run `kipferl add` again for your declared
...
v0.7.0-rc.1 supports ordinary dotted imports:
