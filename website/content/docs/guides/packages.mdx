...
system Python installation. This is a feature of the current source checkout;
the published v0.6 release predates it.
...
The initial catalog includes `tzdata==2025.2` on macOS ARM64 for the recorded
runtime hashes. Its checks cover version constants and the `TZif` headers of
...
With a matching source CLI and a project, install the exact version:
...
Current source builds support ordinary dotted imports:
