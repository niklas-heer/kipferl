...
[install v0.7.0-rc.1](/docs/getting-started/installation#release-candidate) to use
the new project and package commands without installing the contributor tools.
Stable v0.6.0 remains the Homebrew release.
...
The published v0.7.0-rc.1 artifacts were built for all four native targets with
reviewed package catalog evidence against each final runtime binary. The release pipeline checks
...
The broad popularity audit remains a dated screen against its recorded runtime; it is not relabeled as evidence for a new binary.
