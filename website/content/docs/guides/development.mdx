...
Release candidates build all four native targets and generate reviewed package
catalog evidence against each final runtime binary. The release pipeline checks
...
used for each target. The broad popularity audit remains a dated screen against
its recorded runtime; it is not relabeled as evidence for a new binary.
