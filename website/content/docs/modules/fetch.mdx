...
<Callout>
Unlike Python's `requests` library, `fetch` is built into the μcharm runtime.
HTTP framing and HTTPS are maintained through a feature-minimal Ureq, Rustls,
and WebPKI roots stack, so applications do not need a separate package or
system TLS installation.
</Callout>
...
def safe_fetch(url):
    try:
        ...
        if response["status"] >= 400:
            tui.error(f"HTTP {response['status']}")
            return None
        return response
...
if response:
    ...
    tui.success(f"Found user: {data['login']}")
```

## HTTPS Support

HTTPS is fully supported through the runtime's Rustls stack. Certificate
